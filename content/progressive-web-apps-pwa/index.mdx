...
```html showLineNumbers {1-3,4} /science/ title="해당코드의 타이틀을 mdx 에서 작성" caption="mdx 를 쓴다면 rehypePrettyCode는 매우 유용한것같다."
